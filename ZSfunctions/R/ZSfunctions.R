progress_update<-function(file_location_string,progress_status){
  file<-fread(file_location_string)
  library(data.table)
  progress<-data.table(Time=Sys.time(),Status="progress_status")
  file<-rbind(file,progress)
  fwrite(file,file_location_string)
}

emailnotification <- function(sender, recipients, subjectline, msg){
  try(
    send.mail(from = sender,
              ...
              html = TRUE,
              inline = TRUE,
              #this needs to be set   smtp = list(host.name = "", port = ),
              authenticate = FALSE,
              send = TRUE),
    silent = T
  )
...
emailnotificationA <- function(sender, recipients, subjectline, msg, attch){
  try(
    send.mail(from = sender,
              to = recipients,
              subject = subjectline,
              ...
              html = TRUE,
              inline = TRUE,
             #this needs to be set smtp = list(host.name = "10.128.230.22", port = 25),
              attach.files = attch,
              authenticate = FALSE,
              send = TRUE),
    silent = T
